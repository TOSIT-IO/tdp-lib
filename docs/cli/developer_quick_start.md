...
0. make sure to clone tdp-collection at first.


1. install sqlite and create db folder
...
2. set environment variables (tdp cli will read it, and you don't need to mention it afterwards)
...
5. visualise a subgraph of the dag to a specific node (ex : zookeeper_install)

```
user@yourmachine:tdp dag zookeeper_install
```
6. choose a target node in the dag (ex : zookeeper_install) and launch tdp deploy (dry run)  
```
